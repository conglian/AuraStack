...
import com.thinkup.flutter.interstitial.TUAutoLoadInterstitialHelper;
...
public class TUAdInterstitialManger implements HandleThinkUpMethod {
    static Map<String, TUInterstitialHelper> pidHelperMap = new ConcurrentHashMap<>();

    private static class SingletonClassInstance {
        private static final TUAdInterstitialManger instance = new TUAdInterstitialManger();
    }

    private TUAdInterstitialManger() {
    }

    public static TUAdInterstitialManger getInstance() {
    ...
    public boolean handleMethodCall(@NonNull MethodCall methodCall, @NonNull MethodChannel.Result result) {
        ...
        if (!TextUtils.isEmpty(placementID) && TUAutoLoadInterstitialHelper.getInstance().containsPlacementID(placementID)) {
            //全自动加载的操作
            isAutoFlag = true;
    ...
    private void routeNormal(String placementID, @NonNull MethodCall methodCall, @NonNull MethodChannel.Result result) {
        ...
        if (TextUtils.isEmpty(placementID)) {
            MsgTools.printMsg("TUAdInterstitialManger routeNormal: The placementID parameter is null or empty.");
            return;
        }

        TUInterstitialHelper helper = getHelper(placementID);
    ...
    private void routeAutoLoad(String[] placementIDArr,@NonNull MethodCall methodCall, @NonNull MethodChannel.Result result) {
        ...
        TUAutoLoadInterstitialHelper helper = TUAutoLoadInterstitialHelper.getInstance();
    ...
    private TUInterstitialHelper getHelper(String placementId) {
        TUInterstitialHelper helper;

        if (!pidHelperMap.containsKey(placementId)) {
            helper = new TUInterstitialHelper();
