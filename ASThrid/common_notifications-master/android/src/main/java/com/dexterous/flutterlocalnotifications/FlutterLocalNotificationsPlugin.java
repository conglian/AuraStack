      ...
      if (beautifulStyleInformation.image.startsWith("http")) {
        BitmapUtils.createFromNetwork(context, beautifulStyleInformation.image, bitmap -> {
          if (bitmap != null) {
            ...
            int fallbackResId = getDrawableResourceId(context, "as_big_bg");
            small.setImageViewResource(R.id.fln_beauty_notify_image, fallbackResId);
            big.setImageViewResource(R.id.fln_beauty_notify_image, fallbackResId);
          }
          finalSet.run();
        });
      } else {
